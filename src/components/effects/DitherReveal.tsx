// Dither Reveal, Originkit (https://www.originkit.dev/components/dither-reveal)
"use client";
...
export default function DitherReveal(props: DitherRevealProps) {
  ...
  const S = settings({ fit, focusY, ditherStyle, dotSize, revealRadius, revealSoftness, wave, waveSpeed, waveDensity });
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        // A host without CORS headers taints the upload, keep the placeholder.
      }
    };
    ...
    function onMove(e: PointerEvent) {
      ...
      mouse.entered = inside;
      mouse.target = inside ? 1 : 0;
    }
    // Listen on window, not the container. On the Contact page this
    // container sits in a `fixed inset-0 -z-[3]` wrapper, i.e. behind the
    // whole page, and there's also a dark overlay div painted directly in
    // front of it in the same wrapper. Either one alone means a
    // container-scoped listener never actually receives a pointer event
    // (whatever's visually on top catches it first). Pointer events bubble
    // to window regardless of what caught them, so this reaches the same
    // math while actually receiving input from anywhere on the page. Manual
    // bounds-checking above replaces the old pointerenter/pointerleave,
    // which only ever fired for a container-scoped listener.
    ...
    const io = new IntersectionObserver(([entry]) => (inViewRef.current = entry.isIntersecting), { threshold: 0.01 });
