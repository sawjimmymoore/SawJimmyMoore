// Particle Drift — Originkit (https://www.originkit.dev/components/particle-drift)
// Adapted for this Vite app: dropped the Next.js "use client" directive
// (meaningless outside the Next app router, harmless either way, but this
// project doesn't use it anywhere else) and made width/height default to
// the container's own size via ResizeObserver instead of requiring fixed
// pixel props, so it can drop into a hero or footer that isn't 1200x800.
...
export default function ParticleDrift(props: Props) {
    ...
    // Auto-size to the wrapper when no explicit width/height prop is given,
    // via ResizeObserver, so this can drop straight into a hero or footer
    // section without the caller doing any pixel math.
    const [auto, setAuto] = useState({ w: 0, h: 0 })
    const sizeRef = useRef({ w: 0, h: 0 })
    sizeRef.current = { w: num(width, auto.w), h: num(height, auto.h) }
    ...
    useEffect(() => {
        ...
        const ro = new ResizeObserver((entries) => {
            for (const entry of entries) {
                ...
                setAuto({ w: Math.round(w), h: Math.round(h) })
            }
        })
        ro.observe(el)
    ...
    useEffect(() => {
        ...
        // Bound to `window`, not the canvas: the hero/footer both stack real
        // content (headline, buttons, service chips, the sitemap links) on
        // top of this canvas, and a listener on the canvas itself only fires
        // while the pointer is directly over the canvas's own hit-testable
        // area, everywhere a foreground element covers it the drift stops
        // responding, which is exactly the "not interactive" report. Same
        // root cause and same fix as the other scroll/pointer backgrounds in
        // this app: track from `window` and convert to canvas-local space
        // with getBoundingClientRect, so it keeps responding no matter what
